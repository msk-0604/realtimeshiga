...
export const postFormSchema = z
  .object({
    ...
    municipality: z
      .string()
      .refine(
        (v) => (MUNICIPALITIES as readonly string[]).includes(v),
        "市町村を選択してください"
      ),
    address: z
      .string()
  ...
  .superRefine((data, ctx) => {
    ...
    if (hasLat !== hasLng) {
      ctx.addIssue({
        code: "custom",
        message: "緯度と経度はセットで入力してください",
        path: ["latitude"],
      });
    }
    if (data.latitude != null && (data.latitude < 34 || data.latitude > 36)) {
      ctx.addIssue({
        code: "custom",
        message: "緯度が滋賀県の範囲外の可能性があります",
        path: ["latitude"],
      });
    }
    if (data.longitude != null && (data.longitude < 135 || data.longitude > 137)) {
      ctx.addIssue({
        code: "custom",
        message: "経度が滋賀県の範囲外の可能性があります",
        path: ["longitude"],
      });
    }
  });
